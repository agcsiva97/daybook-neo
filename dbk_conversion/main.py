...
names_dbf = DBF(os.path.join(script_dir, 'dbf_files/DBK/siva_names.dbf'), load=True)
transactions_dbf = DBF(os.path.join(script_dir, 'dbf_files/DBK/siva.dbf'), load=True)
...
while True:
    ...
    match user_input:
        case '1':
            type_gen.generate_type_mapping_csv(names_df)
            break
        case '4':
            acc_helper.add_accounts(names_df)
            break
        case '6':
            acc_helper.add_transactions(transactions_df)
            break
        case _:
